...
export function createWebServer(): express.Express {
  ...
  app.get('/api/datasets/:id', (req: Request, res: Response) => {
    ...
    const dataset = store.datasets.find(d => 
      !d.is_deleted && (d.id === targetId || d.code.trim().toUpperCase() === targetCode)
    );
    if (!dataset) {
  ...
  app.post('/api/datasets', (req: Request, res: Response) => {
    ...
    const existingIdx = store.datasets.findIndex(d => 
      d.id === targetId || d.code.trim().toUpperCase() === cleanCode
    );
  ...
  app.put('/api/datasets/:id', (req: Request, res: Response) => {
    ...
    const targetId = String(req.params.id || '').trim();
    const targetCode = targetId.toUpperCase();
    const idx = store.datasets.findIndex(d => 
      !d.is_deleted && (d.id === targetId || d.code.trim().toUpperCase() === targetCode)
    );
    if (idx === -1) {
      res.status(404).json({ success: false, error: 'Dataset tidak ditemukan.' });
  ...
  const executeDeleteDataset = (targetId: string, req: Request, res: Response) => {
    const store = loadBackendStore();
    const cleanId = String(targetId || '').trim();
    const cleanCode = cleanId.toUpperCase();

    const idx = store.datasets.findIndex(d => 
      !d.is_deleted && (d.id === cleanId || d.code.trim().toUpperCase() === cleanCode)
    );
    ...
    const actorId = (req.headers['x-user-id'] as string) || (req.body && req.body.user_id) || 'user-1';
  ...
  app.delete('/api/datasets', (req: Request, res: Response) => {
  ...
  app.post('/api/datasets/:id/delete', (req: Request, res: Response) => {
    executeDeleteDataset(String(req.params.id), req, res);
  });

  app.post('/api/datasets/delete', (req: Request, res: Response) => {
    const id = (req.body && (req.body.id || req.body.dataset_id)) || req.query.id || req.query.dataset_id;
    if (!id) {
      res.status(400).json({ success: false, error: 'ID atau kode dataset wajib disertakan.' });
      return;
  ...
  const executeDeleteRecord = (targetId: string, req: Request, res: Response) => {
    const store = loadBackendStore();
  ...
  app.post('/api/records/:id/delete', (req: Request, res: Response) => {
    executeDeleteRecord(String(req.params.id), req, res);
  });

  app.post('/api/records/delete', (req: Request, res: Response) => {
    const id = (req.body && (req.body.id || req.body.record_id)) || req.query.id || req.query.record_id;
    if (!id) {
      res.status(400).json({ success: false, error: 'ID record wajib disertakan.' });
      return;
