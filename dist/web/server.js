...
export function createWebServer() {
    ...
    app.post('/api/bot/suspend', (req, res) => {
    ...
    app.get('/api/bot/sessions/:phone', (req, res) => {
    ...
    app.post(['/api/bot/reset', '/api/bot/logout'], async (req, res) => {
        try {
    ...
    app.post('/api/bot/refresh-qr', async (req, res) => {
        try {
    ...
    const executeDeleteRecord = (targetId, req, res) => {
        ...
        if (!Array.isArray(store.deleted_record_ids)) {
            store.deleted_record_ids = [];
        }
        if (!store.deleted_record_ids.includes(targetId)) {
            store.deleted_record_ids.push(targetId);
        }
    ...
    app.post('/api/sync/store', (req, res) => {
        ...
        const globalDeletedRecSet = new Set(store.deleted_record_ids);
        ...
        if (Array.isArray(deleted_record_ids) && deleted_record_ids.length > 0) {
            for (const delId of deleted_record_ids) {
                ...
                if (!store.deleted_record_ids.includes(delId)) {
                    store.deleted_record_ids.push(delId);
                    modified = true;
                }
            }
            store.records.forEach(r => {
        ...
        if (Array.isArray(records)) {
            for (const rec of records) {
                ...
                if (globalDeletedRecSet.has(rec.id) ||
                    rec.is_deleted ||
    ...
    app.post('/api/faqs/save', (req, res) => {
        ...
        if (!Array.isArray(store.customFaqs))
            store.customFaqs = [];
        const now = new Date().toISOString();
        const existingIndex = store.customFaqs.findIndex(f => f.pertanyaan.toLowerCase() === (old_pertanyaan || pertanyaan).toLowerCase());
        if (existingIndex >= 0) {
    ...
    app.post('/api/faqs/delete', (req, res) => {
        ...
        const store = loadBackendStore();
        if (Array.isArray(store.customFaqs)) {
            store.customFaqs = store.customFaqs.filter(f => f.pertanyaan.toLowerCase() !== pertanyaan.toLowerCase());
            saveBackendStore(store);
        }
