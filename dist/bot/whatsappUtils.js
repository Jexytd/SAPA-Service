 ...
 */
export function resolveWhatsAppTarget(phoneOrJid) {
    if (!phoneOrJid) {
        ...
        try {
            ...
            if (fs.existsSync(lidMappingPath)) {
                ...
                if (realPhone) {
                    ...
                    if (realClean.startsWith('0'))
                        realClean = '62' + realClean.slice(1);
                    return {
                        phone: realClean,
    ...
    if (trimmed.endsWith('@s.whatsapp.net')) {
        ...
        try {
            ...
            if (fs.existsSync(forwardMappingPath)) {
                ...
                if (mappedLid)
                    fallbackLid = `${String(mappedLid).replace(/[^0-9]/g, '')}@lid`;
            }
        }
        catch { }
        return {
 ...
 */
export async function sendWhatsAppMessageSafe(phoneOrJid, content, options) {
