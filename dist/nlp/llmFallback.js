...
export function buildDynamicContext() {
    try {
        ...
        for (const ds of publishedDatasets) {
            const records = store.records.filter(r => r.dataset_id === ds.id && r.status === DataStatus.PUBLISHED && !r.is_deleted);
            if (records.length === 0)
                continue;
...
export function getSystemPrompt() {
    ...
    return (`Anda adalah asisten AI resmi SAPA BPS (Badan Pusat Statistik) Kabupaten Bangka, Provinsi Kepulauan Bangka Belitung.\n\n` +
...
export async function generateFallbackResponse(userMessage, imageBase64) {
    const aiAnswer = await queryAI(userMessage, imageBase64);
    if (aiAnswer && aiAnswer.length > 10) {
        return aiAnswer;
    }
    return (`Mohon maaf, saat ini data untuk pertanyaan *"${userMessage}"* belum tersedia di sistem kami.\n\n` +
