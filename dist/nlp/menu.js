 ...
 */
export function parseMenuLines(menuText) {
    const items = [];
    if (!menuText)
        return items;
 ...
 */
export function getDynamicMenuItems(faqData) {
        ...
        if (custom && custom.trim()) {
            ...
            if (parsed.length > 0) {
                return parsed.map((p) => {
                    ...
                    const isService = lower.includes('layanan') ||
    ...
    try {
        ...
        if (Array.isArray(store.customFaqs)) {
            for (const cf of store.customFaqs) {
                ...
                if (!p || pLower === 'menu utama' || seenLabels.has(pLower))
                    continue;
 ...
 */
export function findFaqResponseByLabel(label, faqData) {
    if (!faqData || !label)
        return null;
    // 1. Exact match
    if (faqData[label]) {
    ...
    const substringKey = keys.find(k => {
        const kLower = k.trim().toLowerCase();
        if (kLower.length < 3 || kLower === 'menu utama')
            return false;
